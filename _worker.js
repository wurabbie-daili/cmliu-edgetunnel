const Version = '2026-05-13 22:41:02';
/*In our project workflow, we first*/ import //the necessary modules, 
/*then*/ { connect }//to the central server, 
/*and all data flows*/ from//this single source.const Version = '2026-06-17 01:41:21';
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	try {
		const CONFIG_KEY = `config:${hostname}`;//新代码独立HOST更改1
		let configJSON = await env.KV.get(CONFIG_KEY);//新代码独立HOST更改1
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (访问路径 === 'admin/init') {// 重置配置为默认值
						try {
							config_JSON = await 读取config_JSON(env, host, userID, UA, true);
							ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Init_Config', config_JSON, true, false));//是否通知TG更改3(一共有8处，前6处传参数false，后2处传参数true)
							config_JSON.init = '配置已重置为默认值';
							return new Response(JSON.stringify(config_JSON, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
						} catch (err) {
							const errorResponse = { msg: '配置重置失败，失败原因：' + err.message, error: err.message };
						...
						if (访问路径 === 'admin/config.json') { // 保存config.json配置
							try {
								...
								// 保存到 KV
								//await env.KV.put('config.json', JSON.stringify(newConfig, null, 2));//原代码独立host更改3
								const CONFIG_KEY = `config:${host}`;//新代码独立HOST更改3
								await env.KV.put(CONFIG_KEY, JSON.stringify(newConfig, null, 2));//新代码独立HOST更改3
								ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Init_Config', config_JSON, true, false));//是否通知TG传参数false更改3
								return new Response(JSON.stringify({ success: true, message: '配置已保存' }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							} catch (error) {
								console.error('保存配置失败:', error);
								return new Response(JSON.stringify({ error: '保存配置失败: ' + error.message }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
					...
					if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
						...
						// 当前时间 UTC+8(2026-05-08)
						//const nowDateObj = new Date(Date.now() + 8 * 3600 * 1000); // UTC+8
						//const nowDate = `${nowDateObj.getUTCFullYear()}-${String(nowDateObj.getUTCMonth()+1).padStart(2,'0')}-${String(nowDateObj.getUTCDate()).padStart(2,'0')}`;
						//当前时间 UTC+8(精确到时分秒)
						const nowDateObj = new Date(Date.now() + 8 * 3600 * 1000);
						const nowDate =
							`${nowDateObj.getUTCFullYear()}-` +
						...
						if (订阅类型 === 'mixed') {
							...
							const isLoonOrSurge = ua.includes('loon') || ua.includes('surge');
							const { type: 传输协议, 路径字段名, 域名字段名 } = 获取传输协议配置(config_JSON);
							订阅内容 = 完整优选IP.map(原始地址 => {//其他节点LINK(即API节点)放最后更改1
								// 统一正则: 匹配 域名/IPv4/IPv6地址 + 可选端口 + 可选备注
								// 示例: 
								//   - 域名: hj.xmm1993.top:2096#备注 或 example.com
								//   - IPv4: 166.0.188.128:443#Los Angeles 或 166.0.188.128
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	try {
		//let configJSON = await env.KV.get('config.json');//原代码独立HOST更改1
		const CONFIG_KEY = `config:${hostname}`;//新代码独立HOST更改1
		let configJSON = await env.KV.get(CONFIG_KEY);//新代码独立HOST更改1
		if (!configJSON || 重置配置 == true) {
			//await env.KV.put('config.json', JSON.stringify(默认配置JSON, null, 2));//原代码独立HOST更改2
			await env.KV.put(CONFIG_KEY, JSON.stringify(默认配置JSON, null, 2));//新代码独立HOST更改2
			config_JSON = 默认配置JSON;
		} else {
			config_JSON = JSON.parse(configJSON);
		}
